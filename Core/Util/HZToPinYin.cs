...
{ -20319, -20317, -20304, -20295, -20292, -20283, -20265, -20257, -20242, 
 ...
 { "A", "Ai", "An", "Ang", "Ao", "Ba", "Bai", "Ban", "Bang", "Bao", "Bei", 
     ...
     "Zui", "Zun", "Zuo" };
        ...
        public static string Convert(string hzString, bool isCityName)
        {
            //特殊处理。
            if (isCityName && hzString == "重")
                return "chong";
            if (hzString == "婺")
                return "wu";
        ...
        /// <param name="isCityName">是否为城市名</param>
        /// <returns></returns>
        public static string GetFirstPY(string text, bool isCityName)
        {
            var t = string.Empty;
            foreach (var _char in text.ToCharArray())
            {
                var py = HZToPinYin.Convert(_char.ToString(), isCityName);
                if (!string.IsNullOrEmpty(py))
                    t += py.Substring(0, 1).ToUpper();
            }
